    ...
    <main class="flex-grow">
        <section id="job-listings" class="py-12 md:py-16 bg-gradient-to-br from-blue-50 to-gray-50">
            <div class="container mx-auto px-4 md:px-6">
                ...
                <form action="job_listings.php" method="GET" class="mb-12 p-6 bg-white rounded-xl shadow-lg animate-fade-in">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        fadeElements.forEach((el, index) => {
            ...
            }, index * 60); // หน่วงเวลาเล็กน้อยให้แสดงผลไม่พร้อมกัน
        });
        ...
        const observer = new IntersectionObserver((entries, observer) => {
            entries.forEach((entry, index) => {
                if (entry.isIntersecting) {
                    // **จุดแก้ไข** เพิ่มการหน่วงเวลาที่นี่เพื่อให้ช้าลงเหมือนหน้า index
                    setTimeout(() => {
                        entry.target.classList.add('is-visible');
                    }, index * 200); // เพิ่มค่าจาก 100 เป็น 150 (หรือมากกว่า) เพื่อให้หน่วงขึ้น
