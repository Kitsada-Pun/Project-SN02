...
// ตัวแปรสำหรับเก็บค่าฟอร์ม (ค่าเริ่มต้นว่างเปล่าสำหรับผู้ใช้ใหม่)
$username = '';
$first_name = ''; 
$last_name = ''; 
$email = '';
$phone_number = '';
$user_type = '';
$new_password = ''; // เก็บค่ารหัสผ่านใหม่ที่ผู้ใช้กรอก
$confirm_password = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $first_name = trim($_POST['first_name_th'] ?? ''); 
    $last_name = trim($_POST['last_name_th'] ?? ''); 
    ...
    if (empty($username)) {
        ...
        // ตรวจสอบชื่อผู้ใช้ซ้ำ (สำหรับผู้ใช้ใหม่ ไม่ต้องยกเว้น ID)
        $stmt = $condb->prepare("SELECT user_id FROM users WHERE username = ?");
    ...
    if (empty($email)) {
        ...
        // ตรวจสอบอีเมลซ้ำ (สำหรับผู้ใช้ใหม่ ไม่ต้องยกเว้น ID)
        $stmt = $condb->prepare("SELECT user_id FROM users WHERE email = ?");
    ...
    // Password (บังคับสำหรับผู้ใช้ใหม่)
    ...
    if (empty($errors)) {
        // SQL INSERT query เพื่อเพิ่มผู้ใช้ใหม่
        $sql = "INSERT INTO users (username, email, first_name, last_name, phone_number, user_type, password, registration_date, last_login, is_approved) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NULL, 1)";
        
        // หากใน DB เป็น first_name_th, last_name_th, phone ให้ใช้แบบนี้แทน:
        // $sql = "INSERT INTO users (username, email, first_name_th, last_name_th, phone, user_type, password, registration_date, last_login, is_approved) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NULL, 1)";

        $stmt = $condb->prepare($sql);

        if ($stmt === false) {
            error_log("Add User: Error preparing insert statement: " . $condb->error);
            $_SESSION['message'] = [
                'type' => 'error',
                'text' => 'ระบบไม่สามารถเตรียมการเพิ่มข้อมูลได้ กรุณาลองใหม่ภายหลัง.'
            ];
        } else {
            // บันทึกรหัสผ่านเป็น Plain Text ตามที่ผู้ใช้ต้องการสำหรับโปรเจกต์นี้
            $password_to_save = $new_password; 

            // "sssssss" สำหรับ username, email, first_name, last_name, phone_number, user_type, password
            $stmt->bind_param(
                "sssssss", 
                $username,
                $email,
            ...
            if ($stmt->execute()) {
                $_SESSION['message'] = [
                    'type' => 'success',
                    'text' => 'เพิ่มผู้ใช้ใหม่สำเร็จแล้ว!'
                ];
                // Redirect ไปยังหน้าจัดการผู้ใช้หลังจากเพิ่มสำเร็จ
                header('Location: manage_users.php');
                exit();
            } else {
                error_log("Add User: Error executing insert statement: " . $stmt->error);
                $_SESSION['message'] = [
                    'type' => 'error', 
                    'text' => 'เกิดข้อผิดพลาดในการเพิ่มข้อมูลผู้ใช้: ' . htmlspecialchars($stmt->error)
                ];
                // ไม่ต้อง Redirect; จะคงอยู่บนหน้า add_user.php เพื่อแสดงข้อผิดพลาด
            }
            $stmt->close();
        }
    }
...
<head>
    ...
    <title>เพิ่มผู้ใช้ใหม่ - แอดมิน</title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">เพิ่มผู้ใช้ใหม่</h1>
                        </div>
                        <div class="col-sm-6">
            ...
            <main class="content">
                <div class="content-container">
                    <div class="text-center mb-6">
                        <h4 class="font-semibold text-gray-800 kanit-tag header-text">แบบฟอร์มเพิ่มผู้ใช้ใหม่</h4>
                        <p class="text-gray-500 text-sm mt-2">โปรดกรอกข้อมูลผู้ใช้ใหม่ให้ครบถ้วน</p>
                    </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="ant-form-item-control-input-content">
                                <input type="password" id="confirm_password" name="confirm_password" class="form-input" placeholder="ยืนยันรหัสผ่าน" required
                                    data-validation-rules='{"empty": "กรุณายืนยันรหัสผ่าน", "match": "รหัสผ่านและการยืนยันรหัสผ่านไม่ตรงกัน"}'>
    ...
    <script>
        ...
        // Handle messages from PHP sessions (e.g., after redirect from add)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function validateUsername(input) {
                ...
                // Client-side duplicate check (optional, can be done with AJAX)
                // For now, rely on server-side for real duplicate check
            ...
            function validateConfirmPassword(input) {
                ...
                const errorMessage = (input.value === '') ? 
                                       (JSON.parse(input.dataset.validationRules || '{}').empty || 'กรุณายืนยันรหัสผ่าน') :
            ...
            function validateEmail(input) {
                ...
                // Client-side duplicate check (optional, rely on server-side)
            ...
            inputs.forEach(input => {
                ...
                input.addEventListener('input', function() {
                    ...
                    if (inputId === 'username_account') {
                        ...
                        if (confirmPasswordInput) { // ตรวจสอบ confirm password ด้วย หาก password เปลี่ยน
